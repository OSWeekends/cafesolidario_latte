...
import es.osw.cafelatte.domain.utils.ObjectsCompat;
...
public class BezelImageView extends AppCompatImageView {
  ...
  @Override protected boolean verifyDrawable(Drawable who) {
    return ObjectsCompat.equals(who, mBorderDrawable)
        || ObjectsCompat.equals(who, mMaskDrawable)
        || super.verifyDrawable(who);
  }

  @Override public void invalidateDrawable(Drawable who) {
    if (ObjectsCompat.equals(who, mBorderDrawable) || ObjectsCompat.equals(who, mMaskDrawable)) {
      invalidate();
    } else {
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    if (!mCacheValid || width != mCachedWidth || height != mCachedHeight) {
      ...
      if (mMaskDrawable != null) {
        int sc = mCacheCanvas.save();
        mMaskDrawable.draw(mCacheCanvas);
        mMaskedPaint.setColorFilter(
            mDesaturateOnPress && isPressed() ? mDesaturateColorFilter : null);
        mCacheCanvas.saveLayer(mBoundsF, mMaskedPaint,
            Canvas.HAS_ALPHA_LAYER_SAVE_FLAG | Canvas.FULL_COLOR_LAYER_SAVE_FLAG);
        super.onDraw(mCacheCanvas);
        mCacheCanvas.restoreToCount(sc);
      } else if (mDesaturateOnPress && isPressed()) {
        int sc = mCacheCanvas.save();
        mCacheCanvas.drawRect(0, 0, mCachedWidth, mCachedHeight, mBlackPaint);
      ...
      if (mBorderDrawable != null) {
        mBorderDrawable.draw(mCacheCanvas);
      }
    }
